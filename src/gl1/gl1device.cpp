...
setRasterStageOnly(uint32 stage, Raster *raster)
{
	...
	if(raster != rwStateCache.texstage[stage].raster){
		...
		setActiveTexture(stage);
		if(raster){
			assert(raster->platform == PLATFORM_GL1);
			Gl1Raster *natras = PLUGINOFFSET(Gl1Raster, raster, nativeRasterOffset);
	...
	if(zbuf){
		if(natfb->fboMate == zbuf){
			...
			if(natzb->fboMate){
				...
				Gl1Raster *oldfb = PLUGINOFFSET(Gl1Raster, natzb->fboMate, nativeRasterOffset);
				if(oldfb->fbo){
#ifdef PGM_PIPELINE
...
openSDL2(EngineOpenParams *openparams)
{
	...
	memset(&gl1Caps, 0, sizeof(gl1Caps));
...
startSDL2(void)
{
	...
	for(i = 0; profiles[i].gl; i++){
		...
		if(win){
			gl1Caps.gles = profiles[i].gl == SDL_GL_CONTEXT_PROFILE_ES;
			gl1Caps.glversion = profiles[i].major*10 + profiles[i].minor;
			break;
		}
	}
	if(win == nil){
...
#ifdef PGM_PIPELINE
	if (!((gl1Caps.gles ? gladLoadGLES2Loader : gladLoadGLLoader) ((GLADloadproc) SDL_GL_GetProcAddress, gl1Caps.glversion)) ) {
#else // PGM_PIPELINE
	if (!(gladLoadGL((GLADloadfunc) SDL_GL_GetProcAddress, gl1Caps.glversion)) ) {
#endif // PGM_PIPELINE
...
startGLFW(void)
{
	...
	for(i = 0; profiles[i].gl; i++){
		...
		if(win){
			gl1Caps.gles = profiles[i].gl == GLFW_OPENGL_ES_API;
			gl1Caps.glversion = profiles[i].major*10 + profiles[i].minor;
			break;
		}
	}
	...
	/* Init GLAD */
#ifdef PGM_PIPELINE
	if (!((gl1Caps.gles ? gladLoadGLES2Loader : gladLoadGLLoader) ((GLADloadproc) glfwGetProcAddress, gl1Caps.glversion)) ) {
#else // PGM_PIPELINE
...
/*
	...
	for(int i = 0; i < numExt; i++){
		...
		if(strcmp(ext, "GL_EXT_texture_compression_s3tc") == 0)
			gl1Caps.dxtSupported = true;
		else if(strcmp(ext, "GL_KHR_texture_compression_astc_ldr") == 0)
			gl1Caps.astcSupported = true;
//		printf("%d %s\n", i, ext);
	}
*/
#ifdef PGM_PIPELINE
	gl1Caps.dxtSupported = !!GLAD_GL_EXT_texture_compression_s3tc;
	gl1Caps.astcSupported = !!GLAD_GL_KHR_texture_compression_astc_ldr;
#else // PGM_PIPELINE
	gl1Caps.dxtSupported = false;
	gl1Caps.astcSupported = false;
#endif // PGM_PIPELINE

	glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl1Caps.maxAnisotropy);

	if(gl1Caps.gles){
		if(gl1Caps.glversion >= 30)
	...
	glBindBuffer(GL_UNIFORM_BUFFER, ubo_state);
	glBindBufferBase(GL_UNIFORM_BUFFER, gl1::findBlock("State"), ubo_state);
	...
	glBindBuffer(GL_UNIFORM_BUFFER, ubo_scene);
	glBindBufferBase(GL_UNIFORM_BUFFER, gl1::findBlock("Scene"), ubo_scene);
	...
	glBindBuffer(GL_UNIFORM_BUFFER, ubo_object);
	glBindBufferBase(GL_UNIFORM_BUFFER, gl1::findBlock("Object"), ubo_object);
...
Device renderdevice = {
	...
	gl1::clearCamera,
	gl1::showRaster,
	gl1::rasterRenderFast,
	gl1::setRenderState,
	gl1::getRenderState,
	gl1::im2DRenderLine,
	gl1::im2DRenderTriangle,
	gl1::im2DRenderPrimitive,
	gl1::im2DRenderIndexedPrimitive,
	gl1::im3DTransform,
	gl1::im3DRenderPrimitive,
	gl1::im3DRenderIndexedPrimitive,
	gl1::im3DEnd,
#ifdef LIBRW_SDL2
	gl1::deviceSystemSDL2
#else
	gl1::deviceSystemGLFW
#endif
};
