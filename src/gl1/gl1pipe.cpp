...
freeInstanceData(Geometry *geometry)
{
	if(geometry->instData == nil ||
	   geometry->instData->platform != PLATFORM_GL1)
		return;
...
instance(rw::ObjPipeline *rwpipe, Atomic *atomic)
{
	...
	if(geo->instData){
		...
		assert(header->platform == PLATFORM_GL1);
...
render(rw::ObjPipeline *rwpipe, Atomic *atomic)
{
	...
	assert(geo->instData->platform == PLATFORM_GL1);
	if(pipe->renderCB)
...
{
	this->rw::ObjPipeline::init(PLATFORM_GL1);
	this->impl.instance = gl1::instance;
	this->impl.uninstance = gl1::uninstance;
	this->impl.render = gl1::render;
	this->instanceCB = nil;
