...
allocateDXT(Raster *raster, int32 dxt, int32 numLevels, bool32 hasAlpha)
{
	...
	if(gl1Caps.gles && needToReadBackTextures){
...
destroyNativeRaster(void *object, int32 offset, int32)
{
	...
	case Raster::ZBUFFER:
		if(natras->fboMate){
			...
			Gl1Raster *oldfb = GETGL1RASTEREXT(natras->fboMate);
			if(oldfb->fbo){
#ifdef PGM_PIPELINE
...
readNativeTexture(Stream *stream)
{
	...
	Gl1Raster *natras;
	if(flags & 2){
		if(!gl1Caps.dxtSupported){
			tex->destroy();
			RWERROR((ERR_FORMAT_UNSUPPORTED));
			return nil;
		}
		raster = Raster::create(width, height, depth, format | Raster::TEXTURE | Raster::DONTALLOCATE, PLATFORM_GL1);
		allocateDXT(raster, compression, numLevels, flags & 1);
	}else{
		raster = Raster::create(width, height, depth, format | Raster::TEXTURE, PLATFORM_GL1);
	}
	assert(raster);
